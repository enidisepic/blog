...
link:https://datatracker.ietf.org/doc/html/rfc6891[RFC 6891]. I might implement them on my own time so feel free to
...
the same as the underlying structure of requests and responses doesn't change. We will start by implementing the UDP
server and later on add the TCP one. If we get a request we have to resolve recursively via TCP, we do the resolution
via TCP. If we get one via UDP, we do that.
...
=== Understanding the DNS request

image::dns_message_header.svg[]
